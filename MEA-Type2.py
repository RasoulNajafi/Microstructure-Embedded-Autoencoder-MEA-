...
class UNet(nn.Module):
    def __init__(self):
        ...
        self.enc_conv1 = nn.Sequential(nn.Conv2d(1, 128, kernel_size=3, stride=1, padding=1),
                                       nn.BatchNorm2d(128),
                                       nn.ReLU()
                                       )
...
net.load_state_dict(torch.load(r"K:\Najafi\codes\UNet_SH\UNet_V3_type3\net_478_epoch.pkl"))
optimizer.load_state_dict(torch.load(r"K:\Najafi\codes\UNet_SH\UNet_V3_type3\optimizer_478_epoch.pkl"))
device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
net = net.to(device)   
net.eval()

with PdfPages('FluxUNet_V3_typ3.pdf') as pdf:
    for i in range(len(morph_HR)):
        ...
        error_abs=np.abs(FE_H[0].cpu().data.numpy() - test[0, 0, :, :].cpu().data.numpy())
        ...
        data = [
            ...
            flux_magnitude_HR1,
            flux_magnitude_HR2,
            np.abs(flux_magnitude_HR1 - flux_magnitude_HR2)
        ]
...
os.chdir(r"K:\Najafi\codes\UNet_SH\UNet_V3_type3")
with PdfPages('UNet_V3_type3.pdf') as pdf:
    for i in range(len(morph_HR)):
        morphology = morph_HR[i]
        morphology = np.expand_dims(morphology, axis=0)
        morphology = torch.from_numpy(morphology.astype(np.float32))
